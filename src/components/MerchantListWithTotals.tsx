...
export type MerchantListWithTotalsProps = {
  titel: string;
  list: HaendlerItem[];
...
export default function MerchantListWithTotals({
  titel,
  list,
  umsatz,
  loading,
  ...
  return (
    <section className="merchant-admin-list">
      ...
      {loading ? (
        ...
        <p className="merchant-admin-empty">{emptyText}</p>
      ) : (
        <ul>
          {list.map((h) => {
