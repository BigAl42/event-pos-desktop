...
describe("SettlementView", () => {
  ...
  it("calls onBack when back button is clicked", async () => {
    mockGetAbrechnung.mockResolvedValue([]);
    const onBack = vi.fn();
    render(<SettlementView onBack={onBack} />);
    await waitFor(() => {
      expect(screen.getByText(/Noch keine Buchungen/)).toBeInTheDocument();
    });
  ...
  it("writes Notfall-Export JSON and unlocks step 3 only when both exports done", async () => {
    ...
    const modal = screen.getByRole("heading", { name: /Abrechnungslauf abschließen/i }).closest(".settlement-modal");
