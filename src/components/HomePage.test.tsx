...
describe("HomePage (Slave Closeout)", () => {
  beforeEach(() => {
  ...
  it("shows closeout section for slave with status 'nicht angefragt' when no closeout_ok_at", async () => {
    ...
    expect(await screen.findByRole("heading", { name: /Sign out \(cycle complete\)/i })).toBeInTheDocument();
    expect(await screen.findByText(/^not requested$/i)).toBeInTheDocument();
    await waitFor(() => {
      expect(screen.getByText(/Active billing cycle:/i)).toBeInTheDocument();
    });
  });
  ...
  it("shows closeout timestamp and warns when closeout not for active lauf", async () => {
    ...
    expect(await screen.findByText(/OK since/i)).toBeInTheDocument();
    expect(screen.getByText(/\(not for active cycle\)/i)).toBeInTheDocument();
  });
});
