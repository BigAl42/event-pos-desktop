...
import HaendlerverwaltungView from "./HaendlerverwaltungView";
...
vi.mock("../utils/haendlerImportExport", async (importOriginal) => {
  const mod = await importOriginal<typeof import("../utils/haendlerImportExport")>();
  return {
    ...mod,
...
const { exportHaendlerCsv, exportHaendlerExcel } = await import("../utils/haendlerImportExport");
...
describe("HaendlerverwaltungView", () => {
  beforeEach(() => {
  ...
  it("shows header, empty list message and Neuer Händler when list is empty", async () => {
    render(<HaendlerverwaltungView onBack={() => {}} />);
  ...
  it("calls onBack when Zurück is clicked", async () => {
    const onBack = vi.fn();
    render(<HaendlerverwaltungView onBack={onBack} />);
  ...
  it("shows error when createHaendler rejects", async () => {
    ...
    expect(screen.getByText(/DB voll/)).toHaveClass("haendlerverwaltung-error");
  });
  ...
  it("import CSV creates haendler via createHaendler when file has valid CSV", async () => {
    ...
    const { container } = render(<HaendlerverwaltungView onBack={() => {}} />);
