...
export default function AbrechnungView({ onBack }: Props) {
  ...
  async function withPrintable<T>(
    ...
    flushSync(() => {
      root.render(<HaendlerAbrechnungPdf data={dto} />);
    });
    ...
    const printable = container.querySelector<HTMLElement>(".haendler-abrechnung-pdf") ?? container;
    try {
  ...
  return (
    <div className="abrechnung-view">
      ...
      {pdfError && <p className="abrechnung-error">{pdfError}</p>}
      {pdfBatch && (
        <p className="abrechnung-hinweis-lauf">
      ...
      {abschlussOpen && (
        <div className="abrechnung-modal-overlay" onClick={() => setAbschlussOpen(false)}>
          <div className="abrechnung-modal" onClick={(e) => e.stopPropagation()}>
            <div className="abrechnung-modal-header">
            ...
            <p className="abrechnung-modal-subtitle">
              Aktiver Lauf: <strong>{aktuellerLaufName ?? "—"}</strong>
            </p>

            {abschlussError && <p className="abrechnung-error">{abschlussError}</p>}

            <div className="abrechnung-wizard-steps">
            ...
            {abschlussStep === 1 && (
              <section className="abrechnung-wizard-section">
                ...
                {relevantPeers.length === 0 ? (
                  ...
                  <ul className="abrechnung-peer-list">
                    {relevantPeers.map((e) => {
                      const ok =
                        e.connected === true &&
                        (e.closeout_ok_for_lauf_id ?? null) === aktuellerLaufId &&
                      ...
                      return (
                        <li key={e.peer_id} className={ok ? "ok" : "warn"}>
                          <div className="abrechnung-peer-main">
                            <span className="abrechnung-peer-name">{e.name || e.peer_id}</span>
                            <span className={ok ? "abrechnung-badge ok" : "abrechnung-badge warn"}>{ok ? "OK" : "Fehlt"}</span>
                          </div>
                          <div className="abrechnung-peer-sub">
                            <span className="abrechnung-peer-url">{e.ws_url}</span>
                            {e.closeout_ok_at && (
                              <span className="abrechnung-peer-closeout">
                                Closeout: {new Date(e.closeout_ok_at).toLocaleString("de-DE")}
                              </span>
                            )}
                          </div>
                          {!ok && hint && <div className="abrechnung-peer-hint">{hint}</div>}
                        </li>
                      );
                    })}
                  </ul>
            ...
            {abschlussStep === 2 && (
              <section className="abrechnung-wizard-section">
                ...
                <div className="abrechnung-export-grid">
                  <div className="abrechnung-export-card">
                    <div className="abrechnung-export-title">
                      Händler-PDFs (alle) {exportPdfDone && <span className="abrechnung-badge ok">OK</span>}
                    </div>
                    <p className="abrechnung-export-desc">Erstellt pro Händler eine PDF im Zielordner.</p>
                    <button
                      type="button"
                  ...
                  <div className="abrechnung-export-card">
                    <div className="abrechnung-export-title">
                      Notfall-Export (JSON) {exportNotfallDone && <span className="abrechnung-badge ok">OK</span>}
                    </div>
                    <p className="abrechnung-export-desc">Speichert alle Bewegungsdaten des aktiven Laufs als Datei.</p>
