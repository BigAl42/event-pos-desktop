...
export async function elementToPdfBytes(element: HTMLElement): Promise<ArrayBuffer> {
  ...
  if (rect.width <= 1 || rect.height <= 1) {
    throw new Error("PDF-Export fehlgeschlagen: Element hat keine Größe (Render noch nicht fertig?).");
  }
  ...
  if (!ctx) throw new Error("PDF-Export fehlgeschlagen: Canvas-Kontext nicht verfügbar.");
 ...
 */
export async function saveHandbuchPdf(
