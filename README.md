...
Workflow [`.github/workflows/release-desktop.yml`](.github/workflows/release-desktop.yml) läuft bei **Tags** der Form `v*.*.*` (z. B. `v0.2.0`) oder **manuell** („Run workflow“). Zuerst werden dieselben Checks wie in der CI ausgeführt (`cargo test --features test`, `npm run lint`, `npm run test:run`); **erst danach** baut Tauri auf `ubuntu-latest`, `windows-latest` und `macos-latest`. Die Installationsartefakte liegen unter **Actions → Workflow run → Artifacts** (`src-tauri/target/release/bundle/` pro Runner). **`bundle.targets`** in `tauri.conf.json` muss je OS passende Formate bauen (z. B. `deb`/`appimage`/`nsis`/`dmg`/`app`); nur `["app"]` erzeugt praktisch nur das macOS-`.app`. **Windows-CI:** NSIS muss installiert sein (siehe `release-desktop.yml` → `choco install nsis`), sonst bleibt `bundle/` leer. MSI (WiX) ist in der Standardkonfiguration weggelassen, um die CI nicht von WiX abhängig zu machen.
